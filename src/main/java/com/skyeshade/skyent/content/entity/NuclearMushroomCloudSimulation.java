...
public final class NuclearMushroomCloudSimulation {
    ...
    private static final int TORUS_SPAWN_TICKS = 2_400;
    ...
    private static final int STEM_EXTRA_SPAWN_TICKS = 600;
    ...
    private static final int TORUS_FADE_START_TICKS = 2_400;
    ...
    private static final int STEM_LIFETIME_MIN_TICKS = 500;
    private static final int STEM_LIFETIME_RANDOM_TICKS = 600;
    ...
    private void updateGroundHeight(Level level, Vec3 entityPosition) {
        ...
        double targetGroundY = Math.max(height - entityPosition.y, level.getMinBuildHeight() - entityPosition.y);
        if (!Double.isFinite(groundY)) {
            groundY = targetGroundY;
            return;
        }

        double delta = targetGroundY - groundY;
        if (Math.abs(delta) <= 0.35D) {
            groundY = targetGroundY;
        } else {
            groundY += 0.35D * Math.signum(delta);
        }
    }
